...
class DbSettings extends CApplicationComponent
{
	...
	protected $_tableName='parameters_';
    ...
    {        
        ...
        if(!$items)
        {
            ...
            $command=$connection->createCommand('SELECT pa_key, pa_value, pa_type FROM '.$this->getTableName().' WHERE pa_category=:cat');
            ...
            foreach($result AS $row)
                $items[$row['pa_key']] = ($row['pa_type']=='int') ? (int)$row['pa_value'] : $row['pa_value'];
    ...
    {
        $connection=$this->getDbComponent();
        $command=$connection->createCommand('SELECT pa_id FROM '.$this->getTableName().' WHERE pa_category=:cat AND pa_key=:key LIMIT 1');
        $command->bindParam(':cat', $category);
        ...
        if(!empty($result))
            $command=$connection->createCommand('UPDATE '.$this->getTableName().' SET pa_value=:value WHERE pa_category=:cat AND pa_key=:key');
        else
            $command=$connection->createCommand('INSERT INTO '.$this->getTableName().' (pa_category, pa_key, pa_value) VALUES (:cat, :key, :value)');
    ...
    protected function whenRequestEnds()
    {
        ...
        if(count($this->deleteCategoriesFromDatabase)>0)
        {
            ...
            {
                $connection=$this->getDbComponent();
                $command=$connection->createCommand('DELETE FROM '.$this->getTableName().' WHERE pa_category=:cat');
        ...
        if(count($this->deleteItemsFromDatabase)>0)
        {
            foreach($this->deleteItemsFromDatabase AS $catName=>$keys)
            {
                ...
                $connection=$this->getDbComponent();
                $query='DELETE FROM '.$this->getTableName().' WHERE pa_category=:cat AND pa_key IN('.$names.')';
