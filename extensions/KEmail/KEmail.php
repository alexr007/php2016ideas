...
class KEmail extends CApplicationComponent
{
    ...
    */
    ...
    private function needToCreateTable()
    {
        ...
        $checkSql = 'select tablename from pg_tables where tablename =\''.$this->queue_table_name.'\'';
  ...
  additional_headers character varying,
  CONSTRAINT kemail_queue_pkey PRIMARY KEY (id)
);
CREATE INDEX IF NOT EXISTS kemail_queue_idx_1
  ON public.kemail_queue
  USING btree
  ("time");
CREATE INDEX IF NOT EXISTS kemail_queue_idx_2
  ON public.kemail_queue
  USING btree
  (priority);';
